...
aging <- function(dat, at) {
  ...
  if (at == 2) {
    ...
    dat$attr$age <- sample(ages, n, replace = TRUE)
  } else {
...
## ----Ex1epiParam---------------------------------------------------------
param <- param.net(inf.prob = 0.5, rec.rate=0.05, growth.rate = 0.00083, life.expt = 70,
                   omig.rates = c(0.005, 0.001), exp.inmig = 3, imig.risk = 0.1)
init <- init.net(i.num = 5,r.num=0)

## ----Ex1epiControl-------------------------------------------------------
control <- control.net(type = "SIR", nsims = 10, nsteps = 50, 
...
plot(mod, type = "network", at = 50, col.status = TRUE,
      main = "Prevalence at t50")
